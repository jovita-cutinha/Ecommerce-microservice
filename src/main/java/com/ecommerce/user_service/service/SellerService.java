...
public class SellerService {
  ...
  public ApiResponseDto updateProfile(UserRequestDto request, String sellerId) {
    try {
      ...
      Seller seller =
          sellerRepository
              .findByKeycloakId(sellerId)
              .orElseThrow(
                  () -> new UserServiceException("Seller not found", HttpStatus.NOT_FOUND));
  ...
  public ApiResponseDto getUserByToken(String keycloakId) {
    try {
      ...
      throw new UserServiceException(
          "Failed to fetch seller details", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  public UUID getSellerIdByToken(String keycloakId) {
    try {
      // Fetch seller details from MySQL
      Seller seller =
          sellerRepository
